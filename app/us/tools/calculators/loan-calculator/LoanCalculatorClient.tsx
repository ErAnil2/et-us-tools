...
import { CalculatorAfterCalcBanners, CalculatorMobileMrec2, MobileMrec1, MobileMrec2, MrecBanner1 } from '@/components/MRECBanners';
...
const fallbackFaqs = [
  {
    ...
    answer: "A Loan Calculator is a free online tool that helps you calculate and analyze loan-related financial metrics quickly and accurately. It provides instant results to help you make informed financial decisions.",
    order: 1
  },
  {
    id: '2',
    question: "How accurate is this Loan Calculator?",
    answer: "Our Loan Calculator uses standard financial formulas and provides highly accurate results. However, actual results may vary based on specific terms, conditions, and market factors. Always consult with a financial advisor for major decisions.",
    order: 2
  },
  {
    id: '3',
    question: "Is this Loan Calculator free to use?",
    answer: "Yes, this Loan Calculator is completely free to use. There are no hidden charges or registration required. You can use it as many times as you need.",
    order: 3
  },
  {
    id: '4',
  ...
  },
  {
    ...
    answer: "You'll need basic information related to loan such as amounts, rates, time periods, or other relevant values. The calculator will guide you through each required field.",
    order: 5
  }
];

export default function LoanCalculatorClient({ relatedCalculators = defaultRelatedCalculators }: Props) {
  const { getH1, getSubHeading } = usePageSEO('loan-calculator');
  ...
  useEffect(() => {
    ...
    setResults({
      ...
      interestMultiple: result.totalInterest / loanAmount
    });
  ...
  const scenarios = useMemo(() => {
    const current = calculateLoan(loanAmount, interestRate, loanTerm);
    const lowerRate = calculateLoan(loanAmount, Math.max(1, interestRate - 1), loanTerm);
    const shorterTerm = calculateLoan(loanAmount, interestRate, Math.max(5, loanTerm - 10));

    return {
      current: { ...current, rate: interestRate, term: loanTerm, amount: loanAmount },
      lowerRate: {
        ...lowerRate,
        rate: Math.max(1, interestRate - 1),
        term: loanTerm,
        amount: loanAmount,
        savings: current.totalInterest - lowerRate.totalInterest
      },
      shorter: {
        ...shorterTerm,
        rate: interestRate,
        term: Math.max(5, loanTerm - 10),
        amount: loanAmount,
        savings: current.totalInterest - shorterTerm.totalInterest
      }
    };
  }, [loanAmount, interestRate, loanTerm]);
  ...
  const maxChartValue = loanAmount * 1.1;

  const allRelatedCalculators = [
    { href: '/us/tools/calculators/emi-calculator', title: 'EMI Calculator', description: 'Calculate equated monthly installment', icon: '💳' },
    ...
    { href: '/us/tools/calculators/personal-loan-calculator', title: 'Personal Loan', description: 'Personal loan payments', icon: '👤' },
  ...
  const generateAreaPath = () => {
    if (yearlyData.length === 0) return '';
    const linePath = generateBalancePath();
  ...
  const getPointCoords = (index: number) => {
    ...
    const y = chartPadding.top + plotHeight - (yearlyData[index].balance / maxChartValue) * plotHeight;
    return { x, y };
  };
  ...
  return (
    <div className="max-w-[1200px] mx-auto px-2 xs:px-3 sm:px-4 py-4 sm:py-6 md:py-8">
      ...
      <div className="text-center mb-5 sm:mb-6 md:mb-8">
        <h1 className="text-xl xs:text-2xl sm:text-3xl md:text-4xl font-bold text-gray-800 mb-2 sm:mb-3 md:mb-4">{getH1('Loan Calculator')}</h1>
        <p className="text-xs xs:text-sm sm:text-base md:text-lg text-gray-600 px-1 sm:px-2 leading-relaxed">Calculate your monthly loan payments, total interest, and amortization schedule</p>
      </div>
      ...
      <div className="bg-white rounded-lg sm:rounded-xl shadow-lg p-3 xs:p-4 sm:p-6 md:p-8 mb-4 sm:mb-6 md:mb-8">
        <div className="grid lg:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
          ...
          <div className="space-y-3 sm:space-y-4 md:space-y-5">
            ...
            <div>
              <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">Loan Amount</label>
            ...
            <div>
              <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">Interest Rate (% per year)</label>
              <div className="relative">
                <input
                  type="number"
                  value={interestRate}
            ...
            <div>
              <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">Loan Term (Years)</label>
              <input
                type="number"
                value={loanTerm}
            ...
            <div className="bg-blue-50 rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-5 mt-3 sm:mt-4">
              ...
              <div className="grid grid-cols-3 gap-2">
                <button
                  onClick={() => { setLoanAmount(300000); setInterestRate(6.5); setLoanTerm(30); }}
                ...
                <button
                  onClick={() => { setLoanAmount(35000); setInterestRate(5.5); setLoanTerm(5); }}
                  className="px-2 py-2 bg-white border border-blue-200 rounded-lg text-xs font-medium text-blue-700 hover:bg-blue-100 transition-colors"
                >
                  Auto Loan
                </button>
                <button
                  onClick={() => { setLoanAmount(20000); setInterestRate(9); setLoanTerm(5); }}
                  className="px-2 py-2 bg-white border border-blue-200 rounded-lg text-xs font-medium text-blue-700 hover:bg-blue-100 transition-colors"
                >
                  Personal
                </button>
            ...
            {/* MREC Banner below Calculate Button */}
            <MrecBanner1 className="mt-4" />
          </div>

          {/* Right: Results Section */}
          <div>
            <h2 className="text-base sm:text-lg md:text-xl font-semibold text-gray-800 mb-2 sm:mb-3 md:mb-4">Payment Summary</h2>

            <div className="bg-blue-50 border border-blue-200 rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-6 mb-3 sm:mb-4 md:mb-6">
              <div className="text-[10px] xs:text-xs sm:text-sm text-blue-600 mb-0.5 sm:mb-1">Monthly Payment</div>
              <div className="text-xl xs:text-2xl sm:text-3xl md:text-4xl font-bold text-blue-700">{formatCurrencyFull(results.monthlyPayment)}</div>
              <div className="text-[10px] xs:text-xs sm:text-sm text-blue-600 mt-1">{results.totalMonths} monthly payments over {loanTerm} years</div>
            </div>
            ...
            <div className="space-y-1.5 sm:space-y-2 md:space-y-3 mb-3 sm:mb-4 md:mb-6">
              <div className="flex justify-between py-1.5 sm:py-2 border-b border-gray-100 text-xs sm:text-sm md:text-base">
                ...
                <span className="font-semibold text-gray-800">{formatCurrencyFull(loanAmount)}</span>
              </div>
              <div className="flex justify-between py-1.5 sm:py-2 border-b border-gray-100 text-xs sm:text-sm md:text-base">
              ...
              <div className="flex justify-between py-1.5 sm:py-2 border-b border-gray-100 text-xs sm:text-sm md:text-base">
                ...
                <span className="font-semibold text-green-600">{formatCurrencyFull(results.totalPayment)}</span>
              </div>
              <div className="flex justify-between py-1.5 sm:py-2 border-b border-gray-100 text-xs sm:text-sm md:text-base">
                <span className="text-gray-600">Loan Duration:</span>
                <span className="font-semibold text-blue-600">{loanTerm} years ({results.totalMonths} months)</span>
              </div>
            </div>
            ...
            <div className="bg-gray-50 rounded-lg sm:rounded-xl p-2.5 sm:p-3 md:p-4">
              ...
              <div className="grid grid-cols-2 gap-1.5 sm:gap-2 md:gap-4 text-[10px] xs:text-xs sm:text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">${((results.totalInterest / loanAmount) * 1000).toFixed(0)}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* Mobile MREC 1 - After first section */}
      <MobileMrec1 />
      ...
      <div className="bg-white rounded-lg sm:rounded-xl shadow-lg p-3 xs:p-4 sm:p-6 md:p-8 mb-4 sm:mb-6 md:mb-8">
        ...
        <div className="relative overflow-x-auto bg-gradient-to-br from-slate-50 to-gray-100 rounded-lg sm:rounded-xl p-2 sm:p-3 md:p-4 -mx-1 sm:mx-0">
          ...
          >
            <defs>
              <linearGradient id="blueAreaGradientLoan" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="#3b82f6" stopOpacity="0.4" />
                <stop offset="50%" stopColor="#60a5fa" stopOpacity="0.2" />
                <stop offset="100%" stopColor="#93c5fd" stopOpacity="0.05" />
              </linearGradient>
              <linearGradient id="blueLineGradientLoan" x1="0" y1="0" x2="1" y2="0">
                <stop offset="0%" stopColor="#2563eb" />
                <stop offset="100%" stopColor="#3b82f6" />
              </linearGradient>
              <filter id="lineShadowLoan" x="-20%" y="-20%" width="140%" height="140%">
            ...
            {/* Area fill */}
            <path d={generateAreaPath()} fill="url(#blueAreaGradientLoan)" />

            {/* Line */}
            <path d={generateBalancePath()} fill="none" stroke="url(#blueLineGradientLoan)" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" filter="url(#lineShadowLoan)" />
            ...
            >
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold text-emerald-600 ml-auto">{formatCurrencyFull(loanAmount - yearlyData[hoveredYear].balance)}</span>
                </div>
                <div className="flex items-center gap-2 pt-1 border-t border-gray-100">
                  <span className="w-2.5 h-2.5 bg-gradient-to-r from-orange-400 to-orange-600 rounded-full"></span>
                  <span className="text-gray-600">Total Paid:</span>
                  <span className="font-semibold text-orange-600 ml-auto">{formatCurrencyFull(yearlyData[hoveredYear].totalPaid)}</span>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 gap-2 xs:gap-3 sm:gap-4 mb-4 sm:mb-6 md:mb-8">
          <div className="border-2 border-blue-200 bg-blue-50 rounded-lg sm:rounded-xl p-2.5 xs:p-3 sm:p-4 xs:col-span-2 sm:col-span-1">
            ...
            <div className="space-y-1 xs:space-y-1.5 sm:space-y-2 text-[10px] xs:text-xs sm:text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">{scenarios.current.term} yrs</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Monthly:</span>
                <span className="font-medium">{formatCurrencyFull(scenarios.current.monthlyPayment)}</span>
              </div>
            </div>
            <div className="mt-2 xs:mt-3 sm:mt-4 pt-2 sm:pt-3 border-t border-blue-200">
              ...
              <div className="text-base xs:text-lg sm:text-2xl font-bold text-blue-700">{formatCurrencyFull(scenarios.current.totalInterest)}</div>
            </div>
          </div>
          ...
          <div className="border border-gray-200 bg-white rounded-lg sm:rounded-xl p-2.5 xs:p-3 sm:p-4 hover:border-green-300 active:border-green-400 transition-colors">
            ...
            <div className="space-y-1 xs:space-y-1.5 sm:space-y-2 text-[10px] xs:text-xs sm:text-sm">
              ...
              <div className="flex justify-between">
                <span className="text-gray-600">Term:</span>
                <span className="font-medium">{scenarios.lowerRate.term} yrs</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Monthly:</span>
                <span className="font-medium">{formatCurrencyFull(scenarios.lowerRate.monthlyPayment)}</span>
              </div>
            </div>
            <div className="mt-2 xs:mt-3 sm:mt-4 pt-2 sm:pt-3 border-t border-gray-200">
              <div className="text-[8px] xs:text-[10px] sm:text-xs text-gray-500">Total Interest</div>
              <div className="text-base xs:text-lg sm:text-2xl font-bold text-gray-800">{formatCurrencyFull(scenarios.lowerRate.totalInterest)}</div>
          ...
          <div className="border border-gray-200 bg-white rounded-lg sm:rounded-xl p-2.5 xs:p-3 sm:p-4 hover:border-purple-300 active:border-purple-400 transition-colors">
            <div className="flex items-center gap-1.5 sm:gap-2 mb-2 sm:mb-3">
              <span className="text-[10px] xs:text-xs sm:text-sm font-semibold text-gray-800">Shorter Term</span>
              <span className="text-[8px] xs:text-[10px] sm:text-xs bg-purple-500 text-white px-1 xs:px-1.5 sm:px-2 py-0.5 rounded">-10 yrs</span>
            </div>
            <div className="space-y-1 xs:space-y-1.5 sm:space-y-2 text-[10px] xs:text-xs sm:text-sm">
              <div className="flex justify-between">
                <span className="text-gray-600">Rate:</span>
              ...
              <div className="flex justify-between">
                <span className="text-gray-600">Term:</span>
                <span className="font-medium">{scenarios.shorter.term} yrs</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Monthly:</span>
                <span className="font-medium">{formatCurrencyFull(scenarios.shorter.monthlyPayment)}</span>
              </div>
            </div>
            <div className="mt-2 xs:mt-3 sm:mt-4 pt-2 sm:pt-3 border-t border-gray-200">
              <div className="text-[8px] xs:text-[10px] sm:text-xs text-gray-500">Total Interest</div>
              <div className="text-base xs:text-lg sm:text-2xl font-bold text-gray-800">{formatCurrencyFull(scenarios.shorter.totalInterest)}</div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 xs:gap-3 sm:gap-4">
          <div className="bg-green-50 border border-green-200 rounded-lg sm:rounded-xl p-2.5 xs:p-3 sm:p-4">
            ...
            <p className="text-[10px] xs:text-xs sm:text-sm text-gray-600 leading-relaxed">
              Negotiating a 1% lower interest rate could save you {formatCurrencyFull(scenarios.lowerRate.savings)} over the loan term.
          ...
          <div className="bg-purple-50 border border-purple-200 rounded-lg sm:rounded-xl p-2.5 xs:p-3 sm:p-4">
            <div className="flex items-center gap-1.5 sm:gap-2 mb-1.5 sm:mb-2">
              ...
              <span className="font-semibold text-gray-800 text-[10px] xs:text-xs sm:text-sm md:text-base">Shorter Term Benefits</span>
            </div>
            <p className="text-[10px] xs:text-xs sm:text-sm text-gray-600 leading-relaxed">
              Reducing the term by 10 years increases payment to {formatCurrencyFull(scenarios.shorter.monthlyPayment)} but saves {formatCurrencyFull(scenarios.shorter.savings)} in interest.
              Be debt-free faster!
            </p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg xs:rounded-xl shadow-lg p-3 xs:p-4 sm:p-6 md:p-8 mb-4 xs:mb-3 sm:mb-4 md:mb-6 prose prose-gray max-w-none">
        <h2 className="text-base xs:text-lg sm:text-xl font-semibold text-gray-800 mb-2 xs:mb-3 sm:mb-4">Understanding Loan Payments and Amortization</h2>
        <p className="text-xs xs:text-sm sm:text-base text-gray-600 mb-3 xs:mb-4 sm:mb-4 md:mb-6 leading-relaxed">
          A loan payment consists of two fundamental components: principal (the amount you borrowed) and interest (the cost of borrowing). Understanding how these work together is essential for making informed borrowing decisions and potentially saving thousands of dollars over the life of your loan.
        </p>

        <p className="text-xs xs:text-sm sm:text-base text-gray-600 mb-3 xs:mb-4 sm:mb-4 md:mb-6 leading-relaxed">
          The amortization process distributes your payments over the loan term, but not equally. Early in the loan, a larger portion of each payment goes toward interest because the outstanding balance is highest. As you pay down the principal, more of each payment reduces the balance. This is why making extra payments early in a loan has the greatest impact on total interest paid.
        </p>
        ...
        <div className="grid grid-cols-1 xs:grid-cols-3 gap-2 xs:gap-4 sm:gap-4 md:gap-6 mb-4 xs:mb-3 sm:mb-4 md:mb-6">
          <div className="bg-blue-50 rounded-lg xs:rounded-xl p-3 xs:p-4 sm:p-5">
            <h3 className="font-semibold text-blue-800 mb-1 xs:mb-2 text-xs xs:text-sm sm:text-base">Principal</h3>
            <p className="text-[10px] xs:text-xs sm:text-sm text-gray-600 leading-relaxed">The original amount borrowed that must be repaid in full</p>
          </div>
          <div className="bg-orange-50 rounded-lg xs:rounded-xl p-3 xs:p-4 sm:p-5">
            <h3 className="font-semibold text-orange-800 mb-1 xs:mb-2 text-xs xs:text-sm sm:text-base">Interest</h3>
            <p className="text-[10px] xs:text-xs sm:text-sm text-gray-600 leading-relaxed">The cost of borrowing, calculated on the remaining balance</p>
          </div>
          <div className="bg-green-50 rounded-lg xs:rounded-xl p-3 xs:p-4 sm:p-5">
            <h3 className="font-semibold text-green-800 mb-1 xs:mb-2 text-xs xs:text-sm sm:text-base">Amortization</h3>
            <p className="text-[10px] xs:text-xs sm:text-sm text-gray-600 leading-relaxed">The systematic process of paying off debt through scheduled payments</p>
          </div>
        </div>

        <h2 className="text-base xs:text-lg sm:text-xl font-semibold text-gray-800 mb-2 xs:mb-3 sm:mb-4">Types of Loans and Their Features</h2>
        <p className="text-xs xs:text-sm sm:text-base text-gray-600 mb-3 xs:mb-4 sm:mb-4 md:mb-6 leading-relaxed">
          Different loan types serve different purposes. Secured loans like mortgages and auto loans use collateral and typically offer lower rates. Unsecured personal loans have higher rates but don&apos;t require collateral. Fixed-rate loans provide payment predictability, while variable-rate loans may start lower but can fluctuate with market conditions.
        </p>

        <h2 className="text-base xs:text-lg sm:text-xl font-semibold text-gray-800 mb-2 xs:mb-3 sm:mb-4">Loan Payment Formula</h2>
        <p className="text-xs xs:text-sm sm:text-base text-gray-600 mb-2 xs:mb-3 sm:mb-4">This calculator uses the standard amortization formula used by financial institutions:</p>
        ...
        <div className="bg-gray-50 rounded-lg xs:rounded-xl p-3 xs:p-3 sm:p-4 font-mono text-[10px] xs:text-xs sm:text-sm overflow-x-auto mb-4 xs:mb-3 sm:mb-4 md:mb-6">
          <p className="mb-2 xs:mb-3 sm:mb-4">M = P × [r(1 + r)<sup>n</sup>] / [(1 + r)<sup>n</sup> - 1]</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 xs:gap-3 sm:gap-4 md:gap-6">
          <div>
            ...
            <ol className="list-decimal list-inside space-y-1 xs:space-y-1.5 sm:space-y-2 text-xs xs:text-sm sm:text-base text-gray-600">
              ...
              <li>View your monthly payment and total cost breakdown</li>
              <li>Use what-if scenarios to compare options</li>
              <li>Review the amortization schedule for details</li>
            </ol>
          </div>

          <div>
            <h2 className="text-base xs:text-lg sm:text-xl font-semibold text-gray-800 mb-2 xs:mb-3 sm:mb-4">Tips for Managing Your Loan</h2>
            <ul className="space-y-1 xs:space-y-1.5 sm:space-y-2 text-xs xs:text-sm sm:text-base text-gray-600">
              ...
              <li className="flex items-start gap-1.5 xs:gap-2">
                <span className="text-green-500 mt-0.5 flex-shrink-0">•</span>
                <span>Consider bi-weekly payments to make 13 payments yearly</span>
              </li>
              <li className="flex items-start gap-1.5 xs:gap-2">
                <span className="text-green-500 mt-0.5 flex-shrink-0">•</span>
                <span>Shop around and compare APRs from multiple lenders</span>
              </li>
              <li className="flex items-start gap-1.5 xs:gap-2">
                <span className="text-green-500 mt-0.5 flex-shrink-0">•</span>
                <span>Understand all fees including origination and closing costs</span>
              </li>
              <li className="flex items-start gap-1.5 xs:gap-2">
                <span className="text-green-500 mt-0.5 flex-shrink-0">•</span>
                <span>Improve your credit score before applying for better rates</span>
              </li>
              <li className="flex items-start gap-1.5 xs:gap-2">
                <span className="text-green-500 mt-0.5 flex-shrink-0">•</span>
                <span>Refinance when rates drop 0.5-1% below your current rate</span>
              </li>
            </ul>
          </div>
        </div>
      ...
      {/* Mobile MREC 2 - Before FAQ section */}
      <MobileMrec2 />
      ...
      <div className="bg-white rounded-lg xs:rounded-xl shadow-lg p-3 xs:p-4 sm:p-6 md:p-8">
        ...
        <div className="space-y-4 xs:space-y-5 sm:space-y-4 md:space-y-6">
          <div className="border-b border-gray-100 pb-4 xs:pb-5">
            <h3 className="text-sm xs:text-base sm:text-lg font-medium text-gray-800 mb-2">How is my monthly loan payment calculated?</h3>
            <p className="text-xs xs:text-sm sm:text-base text-gray-600 leading-relaxed">
              Your monthly payment is calculated using the amortization formula, which factors in your principal amount, interest rate, and loan term. The formula ensures equal payments throughout the loan while properly allocating amounts between principal and interest. Early payments are interest-heavy, while later payments reduce principal more rapidly.
            </p>
          </div>

          <div className="border-b border-gray-100 pb-4 xs:pb-5">
            <h3 className="text-sm xs:text-base sm:text-lg font-medium text-gray-800 mb-2">What&apos;s the difference between APR and interest rate?</h3>
            <p className="text-xs xs:text-sm sm:text-base text-gray-600 leading-relaxed">
              The interest rate is the cost of borrowing the principal amount, expressed as a percentage. APR (Annual Percentage Rate) includes the interest rate plus other charges like origination fees, closing costs, and mortgage insurance. APR provides a more complete picture of borrowing costs and is required by law for disclosure. Always compare APRs when shopping for loans.
            </p>
          </div>

          <div className="border-b border-gray-100 pb-4 xs:pb-5">
            <h3 className="text-sm xs:text-base sm:text-lg font-medium text-gray-800 mb-2">Should I choose a shorter or longer loan term?</h3>
            <p className="text-xs xs:text-sm sm:text-base text-gray-600 leading-relaxed">
              Shorter terms mean higher monthly payments but significantly less total interest paid. A 15-year mortgage might save $100,000+ in interest compared to a 30-year loan. Longer terms offer lower monthly payments and more cash flow flexibility. Choose based on your budget, financial goals, and whether you plan to make extra payments regardless of term.
            </p>
          </div>

          <div className="border-b border-gray-100 pb-4 xs:pb-5">
            <h3 className="text-sm xs:text-base sm:text-lg font-medium text-gray-800 mb-2">How do extra payments reduce my loan cost?</h3>
            <p className="text-xs xs:text-sm sm:text-base text-gray-600 leading-relaxed">
              Extra payments go directly toward principal, reducing the balance on which interest is calculated. Even small additional payments early in the loan have outsized effects because they compound over time. For example, adding $100 monthly to a $200,000 mortgage at 6.5% could save over $40,000 in interest and pay off the loan 5 years early.
            </p>
          </div>

          <div>
            <h3 className="text-sm xs:text-base sm:text-lg font-medium text-gray-800 mb-2">When should I consider refinancing my loan?</h3>
            <p className="text-xs xs:text-sm sm:text-base text-gray-600 leading-relaxed">
              Consider refinancing when rates drop 0.5-1% below your current rate, when your credit score has improved significantly, or when you want to change loan terms. Calculate the break-even point by dividing closing costs by monthly savings to see how long you need to keep the loan for refinancing to pay off. Also consider how much time remains on your current loan.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg sm:rounded-xl shadow-lg p-3 sm:p-4 md:p-6 mb-4 sm:mb-4 md:mb-6">
        ...
        <FirebaseFAQs pageId="loan-calculator" fallbackFaqs={fallbackFaqs} />
      </div>
    </div>
  );
}
